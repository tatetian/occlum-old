...
impl VMManager {
    pub fn from(addr: usize, size: usize) -> Result<VMManager> {
        ...
        let vmas = {
            ...
            let end = range.end();
            let start_sentry = {
                let range = VMRange::new_empty(start, start)?;
                let perms = VMPerms::empty();
                VMArea::new(range, perms)
            };
            let end_sentry = {
                let range = VMRange::new_empty(end, end)?;
                let perms = VMPerms::empty();
                VMArea::new(range, perms)
            };
    ...
    pub fn munmap(&mut self, addr: usize, size: usize) -> Result<()> {
        ...
        self.apply_perms(&munmap_range, VMPerms::default());
    ...
    pub fn mprotect(&mut self, addr: usize, size: usize, new_perms: VMPerms) -> Result<()> {
        ...
        // The target range must be contained in one VMA
    ...
    fn insert_new_vma(&mut self, insert_idx: usize, new_vma: VMArea) {
        ...
        match (left_mergable, right_mergable) {
            ...
            (true, true) => {
                self.vmas[left_idx].set_end(self.vmas[right_idx].end());
    ...
    fn apply_perms(protect_range: &VMRange, perms: VMPerms) {
        debug_assert!(self.range.is_superset_of(protect_range));
        ...
        unsafe {
            ...
            let prot = protect_range.perms() as i32;
            let sgx_status = occlum_ocall_mprotect(&retval, addr, len, prot);
            assert!(sgx_status == sgx_status_t::SUCCESS && retval == 0);
        }
    }
}
